...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, searchedInput: ''}

  getTheTabItems = tabId => {
    this.setState({activeTab: tabId})
  }

  onChangeInput = event => ({searchInput: event.target.value})

  getTheFilterItem = searchedApps => {
    const {activeTab} = this.state
    const resultItem = searchedApps.filter(each => each.category === activeTab)
    return resultItem
  }

  getTheSearchedResults = () => {
    const {searchedInput} = this.state
    const searchedResults = appsList.filter(eachOne =>
      eachOne.appName.toLowerCase().includes(searchedInput.toLowerCase()),
    )
    return searchedResults
  }

  render() {
    const {activeTab, searchedInput} = this.state
    const searchedResults = this.getTheSearchedResults()
    const filterResults = this.getTheFilterItem(searchedResults)
    ...
    return (
      <div className="app-container">
        <div className="app-store">
          ...
          <ul className="tab-list-button">
            {tabsList.map(eachTab => (
              <TabItem
                tabItemContainer={eachTab}
                key={eachTab.tabId}
                getTheTabItems={this.getTheTabItems}
          ...
          <ul className="apps-list">
            {filterResults.map(eachApp => (
              <AppItem appsContainer={eachApp} key={eachApp.appId} />
            ))}
          </ul>
        </div>
      </div>
